...
## Work Unit Summary
**Status:** Active  
**Problem/Intent:** Reduce failures when models cannot access URLs and improve script authenticity by grounding more content in source material.  
**Constraints:** Keep dependencies lightweight, preserve the existing five-stage pipeline and ESM setup, and avoid breaking current CLI flags or episode storage.  
**Proposed Approach:** Add a content accessibility check and automatic scraping fallback with site-specific extraction patterns, then store scraped content alongside episode metadata. Update script prompts to require sourced analogies, remove artificial hook language, strengthen people/group identification, vary sentence rhythm, and end with concrete details.  
**Open Questions:** Which HTML parser is preferred, should fallback scraping be opt-in or default, and how should scraped content be persisted for reuse?
...
- **Planned Changes:**
  ...
  - Strengthen people/group identification: creators, intended audiences, and comment demographics.
  - Add sentence variety guidance for rhythm and pacing.
  - End with concrete details or verbatim readings instead of forward-looking questions.
- **Impact:** More authentic, source-driven narration with better listener engagement.

### Podcast Landing Page (Medium Priority)
- **Goal:** Establish a public web presence for the podcast.
- **Options:** Static HTML from RSS data, platform distribution, or a simple searchable archive.

### Additional Features
- Episode management and editing utilities.
- Batch processing for multiple URLs.
- Voice configuration per episode type.
- Audio post-processing and enhancement.
- Analytics and usage tracking.

### Implementation Status
- **In Progress:** Scraping fallback system design and prompt updates for content quality.
- **Planned Sequencing:** Accessibility test, site patterns, storage integration, CLI controls, then prompt refinements and validation runs.
